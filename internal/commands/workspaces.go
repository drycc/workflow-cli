...
// WorkspacesSwitch switches the default workspace in the client configuration.
func (d *DryccCmd) WorkspacesSwitch(name string) error {
	s, err := settings.Load(d.ConfigFile)
	if err != nil {
		return err
	}
	...
	_, err = workspaces.Get(s.Client, name)
	if d.checkAPICompatibility(s.Client, err) != nil {
		return err
	}
	...
	d.Printf("Switched to workspace %s\n", name)
