...
func NewWorkspacesCommand(cmdr *commands.DryccCmd) *cobra.Command {
	...
	cmd.AddCommand(workspacesSwitch(cmdr))
...
func workspacesSwitch(cmdr *commands.DryccCmd) *cobra.Command {
	workspaceCompletion := completion.WorkspaceCompletion{ArgsLen: 0, ConfigFile: &cmdr.ConfigFile}
	cmd := &cobra.Command{
		Use:  "switch <name>",
		Args: cobra.ExactArgs(1),
		Example: template.CustomExample(
			"drycc workspaces switch my-workspace",
			map[string]string{
				"<name>": i18n.T("The name of the workspace"),
			},
		),
		Short:             i18n.T("Switch the default workspace"),
		Long:              i18n.T("Switches the default workspace in the client configuration"),
		ValidArgsFunction: workspaceCompletion.CompletionFunc,
		RunE: func(_ *cobra.Command, args []string) error {
			return cmdr.WorkspacesSwitch(args[0])
		},
	}
