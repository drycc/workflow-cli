...
// defaultLanguage is the default language for translations. It is used when no language is specified in the translation key.
...
func loadSystemLanguage() string {
	...
	if langStr == "" {
		klog.V(3).Infof("Couldn't find the LC_ALL, LC_MESSAGES or LANG environment variables, defaulting to %s", defaultLanguage)
		return defaultLanguage
	}
	pieces := strings.Split(langStr, ".")
	if len(pieces) != 2 {
		klog.V(3).Infof("Unexpected system language (%s), defaulting to %s", langStr, defaultLanguage)
		return defaultLanguage
	}
	return pieces[0]
}
...
func findLanguage(root string, getLanguageFn func() string) string {
	...
	klog.V(3).Infof("Couldn't find translations for %s, using %s", langStr, defaultLanguage)
	return defaultLanguage
}
...
func LoadTranslations(root string, getLanguageFn func() string) error {
	...
	translationFiles := []string{
		fmt.Sprintf("%s/%s/LC_MESSAGES/cli.po", root, langStr),
		fmt.Sprintf("%s/%s/LC_MESSAGES/cli.mo", root, langStr),
	}
	...
	gettext.BindLocale(gettext.New("cli", root+".zip", buf.Bytes()))
